    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (username.trim().length() < 6 || username.trim().length() > 20) {
                foundErr  =true;
                errors.setUsernameNotLength("Requirement 6-20 character");
            }
            if (password.trim().length() < 8 || password.trim().length() > 20) {
                foundErr  =true;
                errors.setPasswordNotLength("Requirement 8-20 character");
            } else if (!confirm.trim().equals(password.trim())) {
                foundErr  =true;
                errors.setConfirmNotMatch("Confirm not match");
            }
            if (fullname.trim().length() < 8 || fullname.trim().length() > 40) {
                foundErr  =true;foundErr  =true;
                errors.setFullnameNotLength("Requirement 6-20 character");
            }
            ...
            if (foundErr == true) {
                ...
                boolean result = dao.insertAccount(accounts);
                if (result == true) {
                    url = LOGIN_PAGE;
                }
            }
